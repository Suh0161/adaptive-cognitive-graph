...
class RoutingEntropyMetric:
    ...
    def compute(self) -> float:
        ...
        all_probs = torch.cat(
            self.expert_probs_buffer, dim=0
        )  # (total_samples, n_experts)
...
class ExpertUtilizationTracker:
    ...
    def get_underutilized_experts(self, threshold: float = 0.01) -> List[int]:
        ...
        underutilized = [
            expert_id for expert_id, freq in frequencies.items() if freq < threshold
        ]
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        return {
            ...
            "mean_frequency": (
                sum(freq_values) / len(freq_values) if freq_values else 0.0
            ),
...
class DiagnosticMetrics:
    ...
    def __init__(
        ...
        if not 10 <= log_interval <= 1000:
            raise ValueError(
                f"log_interval must be between 10 and 1000, got {log_interval}"
            )
    ...
    def compute_all(
        self, adapters: Optional[torch.nn.ModuleDict] = None
    ) -> Dict[str, float]:
        """
        Compute all metrics.
