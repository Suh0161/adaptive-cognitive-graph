...
def validate_tensor(
    ...
    if min_val is not None and tensor.min() < min_val:
        raise ValueError(f"{name}: minimum value {tensor.min()} is below {min_val}")

    if max_val is not None and tensor.max() > max_val:
        raise ValueError(f"{name}: maximum value {tensor.max()} is above {max_val}")


def validate_token_ids(
    token_ids: torch.Tensor, vocab_size: int, max_seq_len: int
) -> None:
    """
