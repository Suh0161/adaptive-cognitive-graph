...
def apply_gradient_checkpointing(
    ...
    if checkpoint_experts and hasattr(model, 'experts'):
        if hasattr(model.experts, 'expert_blocks'):
            ...
            logger.info(f"Checkpointed {len(model.experts.expert_blocks)} expert blocks")
...
def verify_trillion_param_constraints(
    model: nn.Module,
    max_active_params: int = 40_000_000_000  # 40B
) -> Dict[str, Any]:
    """
    ...
    for component_name in components:
        if hasattr(model, component_name):
            ...
            if component is not None:
                torch.save(
                    component.state_dict(),
                    checkpoint_dir / f'{component_name}.pt'
                )
...
def load_modular_checkpoint(
    ...
    if components is None:
        components = metadata.get('components', ['encoder', 'router', 'experts', 'verifier', 'fusion'])
...
def estimate_memory_usage(
    ...
    activation_memory_mb = (batch_size * seq_len * d_model * n_layers * 4 * 2) / (1024 ** 2)
