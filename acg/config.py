...
class ACGConfig:
    ...
    def _validate(self):
        ...
        if not 512 <= self.d_model <= 4096:
            raise ValueError(
                f"d_model must be between 512 and 4096, got {self.d_model}"
            )

        # Number of experts between 8 and 64
        if not 8 <= self.n_experts <= 64:
            raise ValueError(
                f"n_experts must be between 8 and 64, got {self.n_experts}"
            )

        # Active experts between 2 and 16
        if not 2 <= self.active_experts <= 16:
            raise ValueError(
                f"active_experts must be between 2 and 16, got {self.active_experts}"
            )

        # Active experts cannot exceed total experts
        if self.active_experts > self.n_experts:
            raise ValueError(
                f"active_experts ({self.active_experts}) cannot exceed n_experts ({self.n_experts})"
            )
        ...
        if not 512 <= self.max_seq_len <= 256000:
            raise ValueError(
                f"max_seq_len must be between 512 and 256000, got {self.max_seq_len}"
            )
        ...
        if not 4 <= self.lora_rank <= 64:
            raise ValueError(
                f"lora_rank must be between 4 and 64, got {self.lora_rank}"
            )

        # Verifier hidden dimension between 256 and 2048
        if not 256 <= self.verifier_hidden <= 2048:
            raise ValueError(
                f"verifier_hidden must be between 256 and 2048, got {self.verifier_hidden}"
            )

        # Verifier threshold between 0.3 and 0.7
        if not 0.3 <= self.verifier_threshold <= 0.7:
            raise ValueError(
                f"verifier_threshold must be between 0.3 and 0.7, got {self.verifier_threshold}"
            )

        # Additional validations for loss weights
        if not 0.001 <= self.balance_loss_weight <= 0.1:
            raise ValueError(
                f"balance_loss_weight must be between 0.001 and 0.1, got {self.balance_loss_weight}"
            )

        if not 0.01 <= self.verify_loss_weight <= 0.1:
            raise ValueError(
                f"verify_loss_weight must be between 0.01 and 0.1, got {self.verify_loss_weight}"
            )

        if not 0.01 <= self.phase_loss_weight <= 0.1:
            raise ValueError(
                f"phase_loss_weight must be between 0.01 and 0.1, got {self.phase_loss_weight}"
            )

        # Gradient clipping validation
        if not 0.5 <= self.grad_clip <= 2.0:
            raise ValueError(
                f"grad_clip must be between 0.5 and 2.0, got {self.grad_clip}"
            )
        ...
        if not 0.85 <= self.ema_decay <= 0.95:
            raise ValueError(
                f"ema_decay must be between 0.85 and 0.95, got {self.ema_decay}"
            )

        # LoRA alpha validation
        if not 0.1 <= self.lora_alpha <= 0.5:
            raise ValueError(
                f"lora_alpha must be between 0.1 and 0.5, got {self.lora_alpha}"
            )
