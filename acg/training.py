...
def compute_routing_balance_loss(expert_probs: torch.Tensor) -> torch.Tensor:
    ...
    max_entropy = torch.log(
        torch.tensor(n_experts, dtype=expert_probs.dtype, device=expert_probs.device)
    )
...
def compute_verification_loss(
    confidence: torch.Tensor, correctness_labels: torch.Tensor
) -> torch.Tensor:
    """
...
def compute_phase_classification_loss(
    phase_logits: torch.Tensor, phase_labels: torch.Tensor
) -> torch.Tensor:
    """
...
def setup_optimizer(model: nn.Module, config: ACGConfig) -> torch.optim.Optimizer:
    """
...
class ACGTrainer:
    ...
    def __init__(
        ...
        self.use_scaler = (
            self.use_mixed_precision and mixed_precision_dtype == torch.float16
        )
        if self.use_scaler:
    ...
    def training_step(
        ...
        with torch.cuda.amp.autocast(
            enabled=self.use_mixed_precision, dtype=self.mixed_precision_dtype
        ):
            # Forward pass with routing info
            logits, confidence, routing_map = self.model(
                token_ids, return_routing_info=True
            )
            ...
            total_loss, loss_dict = compute_total_loss(
                ...
                phase_logits=(
                    routing_map.phase_probs if phase_labels is not None else None
                ),
