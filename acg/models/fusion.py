...
class OutputFusionLayer(BaseModule):
    ...
    def _project_expert_outputs(
        self, expert_outputs: List[torch.Tensor], expert_ids: torch.Tensor
    ) -> List[torch.Tensor]:
        """
    ...
    def _fuse_expert_outputs(
        self, projected_outputs: List[torch.Tensor], verifier_weights: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def forward(
        ...
        if expert_ids is None:
            expert_ids = (
                torch.arange(k, device=encoder_output.device)
                .unsqueeze(0)
                .expand(batch_size, k)
            )
    ...
    def _validate_inputs(
        ...
        for i, expert_out in enumerate(expert_outputs):
            if expert_out.dim() != 3:
                raise ValueError(
                    f"Expert output {i} must be 3D, got {expert_out.dim()}D"
                )
        ...
        if verifier_weights.dim() != 2:
            raise ValueError(
                f"verifier_weights must be 2D, got {verifier_weights.dim()}D"
            )
