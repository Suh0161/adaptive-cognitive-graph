...
class GraphOfExperts(BaseModule):
    ...
    def __init__(self, config: ACGConfig):
        ...
        self.experts = nn.ModuleList(
            [ExpertBlock(config, expert_id=i) for i in range(config.n_experts)]
        )

        # Build DAG topology
        self.adjacency = self._build_dag_topology(
            config.n_experts, config.n_phases, config.dag_topology
        )
    ...
    def _apply_weight_tying(self, n_phases: int):
        ...
        for phase in range(n_phases):
            ...
            for i in range(
                start_idx + 1, min(start_idx + experts_per_phase, len(self.experts))
            ):
                for ref_layer, expert_layer in zip(
                    reference_expert.layers, self.experts[i].layers
                ):
    ...
    def _setup_expert_parallelism(self):
        ...
        for expert_id in range(len(self.experts)):
            ...
            if self.config.enable_cross_edges and str(expert_id) in self.edge_weights:
                self.edge_weights[str(expert_id)] = self.edge_weights[
                    str(expert_id)
                ].to(device)
    ...
    def _process_expert_parallel(
        ...
        def process_single_expert(expert_id: int) -> Tuple[int, torch.Tensor]:
            ...
            neighbor_contrib = self._compute_neighbor_contribution(
                expert_id, expert_output_cache, expert_device
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(unique_experts)
        ) as executor:
            futures = [
                executor.submit(process_single_expert, eid) for eid in unique_experts
            ]
    ...
    def _process_expert_sequential(
        ...
        for expert_id in unique_experts:
            # Compute neighbor contribution
            neighbor_contrib = self._compute_neighbor_contribution(
                expert_id, expert_output_cache, device
            )
    ...
    def forward(
        ...
        if self.expert_parallel and self.expert_devices is not None:
            expert_output_cache = self._process_expert_parallel(
                hidden_states, unique_experts, expert_output_cache
            )
        else:
