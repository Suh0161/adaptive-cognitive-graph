...
class IntentRouter(BaseModule):
    ...
    def __init__(self, config: ACGConfig):
        ...
        self.phase_gates = nn.Parameter(
            torch.ones(config.n_phases, config.n_experts)
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> RoutingMap:
        ...
        # Renormalize
        gated_expert_probs = gated_expert_probs / (gated_expert_probs.sum(dim=-1, keepdim=True) + 1e-10)
