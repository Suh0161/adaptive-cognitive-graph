    ...
    def update_weights(
        ...
        if gradients is not None:
            ...
            grad_A = (
                ...
                else torch.zeros_like(self.lora_A)
            )
            grad_B = (
                self.lora_B.grad
                if self.lora_B.grad is not None
                else torch.zeros_like(self.lora_B)
            )
        ...
        self.weight_delta = (
            self.ema_decay * self.weight_delta + (1 - self.ema_decay) * grad_W
        )
...
def attach_adapters_to_expert(
    expert_block, config: ACGConfig
...
def apply_adapters_to_expert_output(
    expert_output: torch.Tensor, adapters: Dict[str, MemoryAdapter]
) -> torch.Tensor:
    """
