...
class ACGModel(nn.Module):
    ...
    def __init__(self, config: ACGConfig):
        ...
        logger.info(
            f"Estimated active parameters per token: {self.estimate_active_params():,}"
        )
    ...
    def forward(
        ...
        try:
            ...
            expert_outputs = self.experts(
                hidden_states, routing_map.expert_ids, routing_map.expert_gates
            )

            # Step 4: Apply memory adapters
            adapted_outputs = self._apply_adapters(
                expert_outputs, routing_map.expert_ids
            )
    ...
    def _apply_adapters(self, expert_outputs: list, expert_ids: torch.Tensor) -> list:
        """
    ...
    def _merge_expert_outputs(self, expert_outputs: list) -> torch.Tensor:
        ...
        )  # (n_experts, batch, seq_len, d_model)
        ...
        try:
            ...
            try:
                if token_ids.dim() >= 1:
                    ...
                    seq_len = (
                        token_ids.size(1) if token_ids.dim() > 1 else token_ids.size(0)
                    )
                else:
            ...
            logits = torch.zeros(
                batch_size, seq_len, self.config.vocab_size, device=token_ids.device
            )
    ...
    def estimate_active_params(self) -> int:
        ...
        active_expert_params = expert_params * (
            self.config.active_experts / self.config.n_experts
        )
